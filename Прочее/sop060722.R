...
write.table(znach, file='сопутка9000.csv', sep = ";",col.names = NA)
write.table(numbers, file='сопутка90001.csv', sep = ";",col.names = NA)
...
df3rez=cbind(df1_trez,df2_trez,df4rez,df5rez)
...
for (l in (1:41))
{
  ...
  for (m in (1:dim(df3rez)[1])){
    df3rez[m,l+a]=df3rez[m,41+l]/df3rez[m,42]}}
...
for (l in (1:41))
{
  ...
  for (m in (1:dim(df3rez)[1])){
    ...
    if (df3rez[m,42]>2 & df3rez[m,42+l]>2 & df3rez[m,l+a]>0.15){
      df3rez[m,tyu+l]=1}}}

#####
for (m in (1:dim(df3rez)[1])){    
  df3rez$summa[m]=sum(df3rez[m,(124:(124+40))])-1}
...
for (l in (1:41))
{
  ...
  for (m in (1:dim(df3rez)[1])){
    if (df3rez$summa[m] < 3){
      if (df3rez[m,123+l]==0){
        if (df3rez[m,42+l]>10 & df3rez[m,l+a]>0.05){
          df3rez[m,123+l]=1}}}}}
...
write.table(df_final23[,1:2], file=paste0('сопутка_',format(Sys.time(), "%d %m %Y"),'.csv'), sep = ";",col.names = NA)
